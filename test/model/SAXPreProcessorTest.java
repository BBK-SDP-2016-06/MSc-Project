...
/**
 * Testing class for the SaxPreProcessor class of the classificationApp.model.preprocessing package.
...
public class SAXPreProcessorTest {
    ...
    public void testDiscretizeWithEmptyListFails() {
        PreProcessor processor = new SAXPreProcessor(1, 2);
        processor.discretize(new TimeSeriesImpl(Optional.empty(), new ArrayList<>()));
    }

    @Test (expected = IllegalArgumentException.class)
    public void testDiscretizeWithListSizeSmallerThanFrameCountFails() {
        PreProcessor processor = new SAXPreProcessor(3, 2);
        processor.discretize(new TimeSeriesImpl(Optional.empty(), Arrays.asList(1.1, 2.1)));
    }
