...
/**
 * Decouples model.exception handling and throwing of exceptions from model.preprocessing package.
    ...
    public static void compareFrameAndDataSize(List<Double> data, long frames) {
        if (frames > data.size()) {
            throw new IllegalArgumentException("Frame count (" + frames + ") " +
                    "cannot be greater than size of model.data sample (" + data.size() + ").");
        }
    }
}
