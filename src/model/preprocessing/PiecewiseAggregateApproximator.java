...
/**
 * Reduces a time series model.data sample down to an approximation by splitting
 * the series into a specified fixed number of frames. A mean value is then
 * calculated from the time series model.data that falls within each frame, and
 * these new values then make up the approximate model.data sample. The frame value
