...
/**
 * Charged with the opening and reading of input model.data files that are to
 * be used in the model.classification process, converting the csv / txt
 * streams into a model.data structure recognised as by the application.
 * Created by George Shiangoli on 20/07/2016.
 */
public interface FileReader {

    /**
     * Returns the number of time series model.data samples in the file.
     * @return the number of time series model.data samples in the file.
     */
    int getTimeSeriesCount();

    /**
     * Returns the entire list of TimeSeries model.data objects obtained from
     * reading the input file.
     * @return list of TimeSeries objects representing all time series
     * model.data samples within the given file.
     */
    List<TimeSeries> getDataSet();

    /**
     * Returns a TimeSeries object from the list model.data structure constructed
     * from the input file.
     * @param index the location within the list of TimeSeries objects that
     *              the required time series model.data should be extracted from.
